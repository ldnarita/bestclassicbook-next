...
// 1–2 sentence SEO intro (generic but works)
function seoIntro(title: string, author?: string) {
  const by = author ? ` of ${author}’s` : "";
  return `Looking for a clear and spoiler-light ${title} summary? This guide covers the plot, main characters, key themes, and the deeper meaning${by} classic, so you can understand the story fast.`;
}

function ensureH1(md: string, title: string) {
  // Replace first H1 or add one at top
  const target = `# ${title} Summary (Plot, Themes & Analysis)`;
  if (md.match(/^#\s+/m)) {
    return md.replace(/^#\s+.*$/m, target);
  }
  return `${target}\n\n${md}`;
}

function hasSeoIntro(md: string) {
  // assume intro exists if a paragraph immediately after H1 exists
  const m = md.match(/^# .+\n\n(.+)\n/);
  return Boolean(m && m[1] && !m[1].startsWith("**") && !m[1].startsWith("---"));
}

function insertSeoIntro(md: string, intro: string) {
  // Insert right after H1 block
  return md.replace(/^(# .+\n)/, `$1\n${intro}\n`);
}
...
function ensureSectionHeadings(md: string, title: string) {
  // Optional: normalize generic headings to SEO headings
...
function getTitleFromMd(md: string, filename: string) {
  ...
  if (m) {
    // if already "X Summary (...)", strip suffix to get base title
    return m[1].replace(/\s+Summary.*$/i, "").trim();
  }
